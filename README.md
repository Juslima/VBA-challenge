...
The first step in the project is to write a VBA script that loops through each row of the data and calculates the yearly change in price and the percentage change in price for each stock. The yearly change is calculated by subtracting the opening price at the beginning of the year from the closing price at the end of the year. The percentage change is then calculated as the yearly change.
